...
const fs = require('fs');
const apiKeysFile = 'apiKeys.json'; // File to store API keys
const git = simpleGit();
...
const mongoURI = 'mongodb+srv://reigerwesley:System1153@soccervortex.e7r8v.mongodb.net/?retryWrites=true&w=majority&appName=soccervortex';
...
app.use(session({
    secret: 'wesleystephanieomaenmaendavy',
    resave: false,
    saveUninitialized: false,
    store: MongoStore.create({ mongoUrl: mongoURI }), // Use your MongoDB connection string
...
app.post('/admin/add-api-key', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        await git.add(apiKeysFile);
        await git.commit('Add new API key');
